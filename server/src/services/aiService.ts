...
  async answerJobQuestion(question: string, job: IJob, user: IUser): Promise<string> {
    try {
      const prompt = `
...
  async generateCustomAnswers(questions: string[], job: IJob, user: IUser): Promise<Array<{question: string, answer: string}>> {
    const answers = [];
...
  async optimizeApplication(job: IJob, user: IUser): Promise<OptimizationSuggestion> {
    try {
      const prompt = `
...
  private generateFallbackCoverLetter(job: IJob, user: IUser): string {
...
  private generateFallbackAnswer(question: string, job: IJob, user: IUser): string {
...
  private generateFallbackOptimization(job: IJob, user: IUser): OptimizationSuggestion {
